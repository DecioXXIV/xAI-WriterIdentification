import torch, glob, os, cv2, numbers, random, sys, itertools, json
...
class NRandomCrop(object):
    def __init__(self, size, n=1, padding=0, pad_if_needed=False):
        if isinstance(size, numbers.Number): self.size = (int(size), int(size))
        else: self.size = size
...
class Confidence_Test_DataLoader(Base_DataLoader):
    def compose_transform(self, n_pair_test_crops = 250):
        ...
        match self.phase:
            ...
            case 'random': return pair_test_randoms_transforms
...
class save_results():
    ...
    def save_checkpoints(self, ep_loss, min_loss, model, optimizer, name, check=True):
        if check is True:
            if ep_loss <= min_loss:
                torch.save({
            ...
            torch.save({
                'model_state_dict': model.state_dict(),
                'optimizer_state_dict': optimizer.state_dict(),
                'loss': ep_loss}, self.checkpoint_path + os.sep + self.test_name + 'new_last_checkpoint.pth')
            
            if os.path.exists(f"{self.checkpoint_path + os.sep + self.test_name}last_checkpoint.pth"):
                os.system(f"rm -r {self.checkpoint_path + os.sep + self.test_name}last_checkpoint.pth")
            
            os.system(f"mv {self.checkpoint_path + os.sep + self.test_name}new_last_checkpoint.pth {self.checkpoint_path + os.sep + self.test_name}last_checkpoint.pth")
...
class Trainer():
    ...
    def train_model(self):
        ...
        if os.path.exists(f"{self.checkpoint_path}/../{self.test_name}train_loss.pkl"): 
            with open(f"{self.checkpoint_path}/../{self.test_name}train_loss.pkl", 'rb') as f:
                train_loss = pkl.load(f)
        else: train_loss = list()
        
        if os.path.exists(f"{self.checkpoint_path}/../{self.test_name}val_loss.pkl"): 
            with open(f"{self.checkpoint_path}/../{self.test_name}val_loss.pkl", 'rb') as f:
                val_loss = pkl.load(f)
        else: val_loss = list()
        
        if os.path.exists(f"{self.checkpoint_path}/../{self.test_name}train_accuracy.pkl"): 
            with open(f"{self.checkpoint_path}/../{self.test_name}train_accuracy.pkl", 'rb') as f:
                train_acc = pkl.load(f)
        else: train_acc = list()
        
        if os.path.exists(f"{self.checkpoint_path}/../{self.test_name}val_accuracy.pkl"): 
            with open(f"{self.checkpoint_path}/../{self.test_name}val_accuracy.pkl", 'rb') as f:
                val_acc = pkl.load(f)
        else: val_acc = list()
        ...
        first_epoch = 1
        if "EPOCHS_COMPLETED" in self.exp_metadata: first_epoch = self.exp_metadata["EPOCHS_COMPLETED"] + 1
        
        for epoch in range(first_epoch, self.num_epochs + 1):
            ...
            min_loss_t = sr.save_checkpoints(epoch_loss, min_loss_t, self.model, optimizer, 'train', check=True)
            ...
            min_loss_v = sr.save_checkpoints(epoch_val_loss, min_loss_v, self.model, optimizer, 'val', check=True)
            
            _ = sr.save_checkpoints(epoch_loss, min_loss_t, self.model, optimizer, None, check=False)
