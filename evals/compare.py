...
def pair_explanations_test(baseline_id, retrained_id, xai_algorithm):
    ...
    test_instance_names = os.listdir(f"{XAI_ROOT}/explanations/patches_{BLOCK_W}x{BLOCK_H}_removal/{R_EXP_DIR}")
    test_instance_names.remove("rgb_train_stats.pkl")

    for name in test_instance_names:
        base_scores_path = f"{XAI_ROOT}/explanations/patches_{BLOCK_W}x{BLOCK_H}_removal/{B_EXP_DIR}/{name}/{name}_scores.pkl"
        ret_scores_path = f"{XAI_ROOT}/explanations/patches_{BLOCK_W}x{BLOCK_H}_removal/{R_EXP_DIR}/{name}/{name}_scores.pkl"
