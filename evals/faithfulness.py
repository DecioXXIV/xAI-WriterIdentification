...
if __name__ == '__main__':
    ...
    EXP_DIR = EXP_METADATA[f"{XAI_ALGORITHM}_base_{SURROGATE_MODEL}_METADATA"]["DIR_NAME"]
    PATCH_WIDTH = EXP_METADATA[f"{XAI_ALGORITHM}_base_{SURROGATE_MODEL}_METADATA"]["PATCH_DIM"]["WIDTH"]
    PATCH_HEIGHT = EXP_METADATA[f"{XAI_ALGORITHM}_base_{SURROGATE_MODEL}_METADATA"]["PATCH_DIM"]["HEIGHT"]
    ...
    exp_eval_directory = f"{EVAL_ROOT}/faithfulness/{TEST_ID}/{XAI_ALGORITHM}_base_{SURROGATE_MODEL}"
    ...
    for i, mask_rate in enumerate(mask_rates):
        ...
        else: mask_test_instances(instances, paths, TEST_ID, EXP_DIR, mask_rate, MASK_MODE, PATCH_WIDTH, PATCH_HEIGHT, XAI_ALGORITHM, SURROGATE_MODEL, EXP_METADATA)
        
        mask_rate_performances = test_model(model, DEVICE, TEST_ID, CLASSES, EXP_METADATA, XAI_ALGORITHM, mask_rate, MASK_MODE, exp_eval_directory)
    ...
    if os.path.exists(faithfulness_saliency_path) and os.path.exists(faithfulness_random_path):
        produce_faithfulness_comparison_report(TEST_ID, MASK_STEP, MASK_CEIL, exp_eval_directory)