...
class feature_analysis():
    ...
    def compute_features(self):
        ...
        print(f"*** {self.phase} WRITERS:")
        for k, v in idx_to_c.items():
            print(f"\t Author {v} -> Index = {k}")

        dict_aut = {k: [] for k in range(len(self.authors))}
        
        # c_to_idx -> Pairs: (author_name, author_idx)
        # idx_to_c -> Pairs: (author_idx, author_name)
        # dict_aut -> Pairs: (author_idx, [])

        output_rows, output_cols = 0, 0
        for data, target in tqdm(set_, desc=f'Computing Features for the {self.phase} set, batch-per-batch'):
        ...
        print(f"*** {self.phase} Features Computed! ***")
        print(f"{self.phase} Output shape = ({output_rows}, {output_cols})")
        
        new_dict_aut = {idx_to_c[k]: dict_aut[k] for k in list(idx_to_c.keys())}
        # new_dict_aut -> Pairs: (author_name, [features])
        ...
        for author in tqdm(range(len(self.authors))):
            ...
            # all_dataframes -> Pairs: (author_name, dataframe)
        ...
        # all_dataframes -> Pairs: (author_name, features_dataframe)

        for author in tqdm(range(len(self.authors)), desc='Concatenating the DataFrames for each Author'):
            if author == 0:
                df = all_dataframes[self.authors[author]]
            else:
                df = df._append(all_dataframes[self.authors[author]], ignore_index=True)
        
        # df -> DataFrame with all the features and the corresponding "author_name" ('Nome')
        ...
        # cat_dict -> Pairs: (author_name, author_idx)
        # cat_list -> List of author_idx (the "author_idx" describes the author of the corresponding feature vector in the DataFrame)

        embeddings_to_save = deepcopy(df)
        embeddings_to_save.pop('Nome')

        embeddings_to_save.to_csv(self.data_path + os.sep + 'Test_' + self.test_ID + '_' + self.test_type + '_embeddings_' + self.phase + '.csv', 
            index=False, 
            sep=";", 
            header = True)
    ...
    def produce_plots(self, df, cat_list, dim_red):
        ...
        assert dim_red in ['PCA-2D', 'PCA-3D', 'tSNE-2D', 'tSNE-3D', 'UMAP-2D', 'UMAP-3D'], 'Wrong dimensionality reduction technique; insert either "PCA-2D" or "PCA-3D" or "t-SNE-2D" or "t-SNE-3D" or "UMAP-2D" or "UMAP-3D"'
        ...
        if dim_red == 'PCA-2D':
            ...
            x_red = pca.fit_transform(x)
        elif dim_red == 'tSNE-2D':
            x_red = TSNE(n_components = 2).fit_transform(x)
        elif dim_red == 'tSNE-3D':
            x_red = TSNE(n_components = 3).fit_transform(x)
        elif dim_red == 'UMAP-2D':
            reducer = umap.UMAP(n_components=2)
            x_red = reducer.fit_transform(x)
        elif dim_red == 'UMAP-3D':
            reducer = umap.UMAP(n_components=3)
            x_red = reducer.fit_transform(x)
        else:
            pass

        print(f"\t {dim_red} computed!")

        _, dimensions = dim_red.split('-')
        ...
        if dimensions == '2D':
            ...
            principalDf = pd.DataFrame(data=x_red, columns=['x1', 'x2', 'x3'])
            principalDf.insert(0, 'Nome', df['Nome'])
            principalDf.insert(1, 'Categoria', cat_list)
            ...
            for i in range(len(self.authors)):
                ...
                p1 = np.mean(principalDf[principalDf['Nome'] == name]['x1'])
                p2 = np.mean(principalDf[principalDf['Nome'] == name]['x2'])
                p3 = np.mean(principalDf[principalDf['Nome'] == name]['x3'])
                centroid_name.append(name)
                centroids_x.append(p1)
                centroids_y.append(p2)
                centroids_z.append(p3)

            cmap_dict = {'train': 'twilight', 'val': 'viridis'}

            fig = plt.figure(figsize=(15, 15))
            ax = fig.add_subplot(111, projection='3d')
            sc = ax.scatter(principalDf['x1'], principalDf['x2'], principalDf['x3'], c=principalDf['Categoria'],
                        s=50, cmap=cmap_dict[self.phase], edgecolors='none')

            size = 81
            lp = lambda i: ax.plot([], [], [], color=sc.cmap(sc.norm(i)), ms=np.sqrt(size), mec='none',
                               label=list(dict.fromkeys(list(principalDf['Nome'])))[i], ls='', marker='o')[0]

            handles = [lp(i) for i in range(len(list(set(principalDf['Categoria']))))]
            plt.legend(handles=handles, fontsize=15, loc='best')

            ax.scatter(centroids_x, centroids_y, centroids_z, c='black', s=50, marker='x')
            ...
            for j in range(len(centroids_x)):
                ax.text(centroids_x[j], centroids_y[j], centroids_z[j], centroid_name[j], color='black', fontsize=20)

            plt.savefig(self.data_path + os.sep + 'Test_' + self.test_ID + '_' + self.test_type + '_' + dim_red + '_' + self.phase + '.png')


        print(f"\t {dim_red} plot saved!")

        # reduced_instances_report = pd.DataFrame(data = x_red, columns = [f'x{i}' for i in range(1, x_red.shape[1]+1)])
        
        # instance_names, authors = [], set(df['Nome'].values.tolist())
        # for author_name in authors:
        #     instances = os.listdir(self.data_dir + os.sep + author_name)
        #     instance_names.extend(instances)
        
        # reduced_instances_report.insert(0, 'Instance', instance_names)
        # reduced_instances_report.insert(1, 'Author', df['Nome'])

        # reduced_instances_report.to_csv(self.data_path + os.sep + 'Test_' + self.test_ID + '_' + self.test_type + '_' + dim_red + '_' + self.phase + '_report.csv', sep=';', header=True, index=False)

        # print(f"\t {dim_red} report generated!\n")
    ...
    def __call__(self):
        ...
        print(f"*** {self.phase}, PHASE 1: Features Computation ***")
        cat_list, df = self.compute_features()

        print(f"*** {self.phase}, PHASE 2: Dimensionality Reduction and Report Generation ***")
        for dim_red in ['PCA-2D', 'PCA-3D', 'tSNE-2D', 'tSNE-3D', 'UMAP-2D', 'UMAP-3D']:
            self.produce_plots(df, cat_list, dim_red)

        print(f"*** {self.phase}, PHASE 3: Mean Average Precision Computation ***")
