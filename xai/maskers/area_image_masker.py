...
class AreaImageMasker:
    ...
    def __call__(self):
        self.logger.info(f"*** Experiment: {self.test_id} -> BEGINNING OF AREA MASKING PROCESS ***")
        ...
        for inst, path in zip(self.instances, self.paths):
            ...
            src_path = f"{self.INSTANCE_DIR_MODE_RATE}/{inst_name}_masked_area_{self.mask_rule}_{self.mask_rate}{inst_filetype}"

            # 'train' Instances are masked to be then used for "ret" Experiments: 
            if self.inst_set == "train": dest_dir = f"{DATASET_ROOT}/{self.dataset}/{c}-{self.test_id}_{MODEL_TYPE}_masked_area_{self.mask_rule}_{self.mask_rate}_{self.xai_algorithm}"

            # 'test' Instances are masked to be then used for the "Faithfulness" evaluation
            elif self.inst_set == "test": dest_dir = f"{EVAL_ROOT}/faithfulness/{self.test_id}/{self.xai_algorithm}_{self.xai_mode}_{self.surrogate_model}/test_set_masked_area_{self.mask_rule}_{self.mask_rate}/{c}"
...
### Subclasses ###
class AreaSaliencyMasker(AreaImageMasker):
    ...
    def mask_full_instance(self, full_img_name, full_img_path): 
        ...
        if self.verbose: self.logger.info(f"Processing Area Saliency Masking for Instance: {full_img_name}")
        ...
        ROOT_OF_MASKINGS = f"{XAI_ROOT}/masked_images/{self.exp_dir}/area_saliency"
        self.INSTANCE_DIR = f"{ROOT_OF_MASKINGS}/{full_img_name}"
        os.makedirs(self.INSTANCE_DIR, exist_ok=True)
        
        MASK_METADATA_PATH = f"{ROOT_OF_MASKINGS}/{self.inst_set}_area_saliency_{self.mask_rate}_{self.xai_algorithm}-metadata.json"
        MASK_METADATA = None
        ...
        full_img_masked.save(f"{self.INSTANCE_DIR_MODE_RATE}/{full_img_name}_masked_area_saliency_{self.mask_rate}{full_img_type}")
        
        if self.verbose: self.logger.info(f"End of Area Saliency Masking Process for the current Instance -> Masked Patches: {masked_patches}\n")
...
class AreaRandomMasker(AreaImageMasker):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
    ...
    def mask_full_instance(self, full_img_name, full_img_path):
        ...
        """
        if self.verbose: self.logger.info(f"Processing Area Random Masking for Instance: {full_img_name}")
        full_img = Image.open(full_img_path)
        ...
        ROOT_OF_MASKINGS = f"{XAI_ROOT}/masked_images/{self.exp_dir}/area_random"
        self.INSTANCE_DIR = f"{ROOT_OF_MASKINGS}/{full_img_name}"
        os.makedirs(self.INSTANCE_DIR, exist_ok=True)
        
        MASK_METADATA_PATH = f"{ROOT_OF_MASKINGS}/{self.inst_set}_area_random_{self.mask_rate}_{self.xai_algorithm}-metadata.json"
        MASK_METADATA = None
        if not os.path.exists(MASK_METADATA_PATH):
            MASK_METADATA = {"INSTANCES": dict()}
            save_metadata(MASK_METADATA, MASK_METADATA_PATH)
        ...
        full_img_masked_tensor_copy = deepcopy(full_img_to_mask_tensor)
        full_img_masked = T.ToPILImage()(full_img_masked_tensor_copy)
        full_img_masked.save(f"{self.INSTANCE_DIR_MODE_RATE}/{full_img_name}_masked_area_random_{self.mask_rate}{full_img_type}")
        
        if self.verbose: self.logger.info(f"End of Area Random Masking Process for the current Instance -> Masked Patches: {masked_patches}\n")
