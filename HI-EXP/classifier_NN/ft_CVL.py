...
def get_args():
    parser = ArgumentParser(description="Hyperparameters for the Fine-Tuning on 'CVL' Dataset", add_help=True)
    ...
    for c in CLASSES:
        ...
        for instance in train:
            for i in range(0, TRAIN_REPLICAS):
                os.system(f"cp {SOURCE_DATA_DIR}/{c}/{instance} {DATASET_DIR}/train/{c}/{instance[:-4]}_cp{i+1}.png")
        
        for instance in val:
            os.system(f"cp {SOURCE_DATA_DIR}/{c}/{instance} {DATASET_DIR}/val/{c}/{instance[:-4]}_cp{i+1}.png")
        
        for instance in test:
            os.system(f"cp {SOURCE_DATA_DIR}/{c}/{instance} {DATASET_DIR}/test/{c}/{instance[:-4]}.png")
